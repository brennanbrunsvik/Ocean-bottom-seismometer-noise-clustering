...
levels_break = 2; 
coh_or_spec = 'coh'; % coherance (coh) or spectra (spec)

OBS_TableParams;
datswitch = 2; % Look in prep data
...
if loopOptimizePenalty(1); 
    ...
    [     eachPenaltyTempTrue,...
          ...
          eachPenaltyTempTotN,...
          penBreak...
          ] = optimize_penalty(dataSets.data1, dat, fnew, penaltyFunction, ...
              thisax, and(showPenalOptim, (i==1)), 100); 
...
if j > 0; % Execute this code if we are going in 2 deep. Combine the name and boolean for our second "layer". e.g. "shallow + seismometer = T240"
    ...
    [     eachPenaltyTempTrue,...
              ...
              eachPenaltyTempTotN,...
              penBreak...
              ] = optimize_penalty(dataSets.data2(thisbool), dat(thisbool,:), ...
                    fnew, penaltyFunction, ...
                    thisax, and(showPenalOptim, (j==1)), 100); 
          [minPen, ipenBreak] = min(eachPenaltyTempTot); 
          penBreakBest = penBreak(ipenBreak); 
          cut2 = penBreakBest
          if j == 1; 
              bool_to_add = (dataSets.data2  < penBreakBest)'; % 
          elseif j == 2; 
              bool_to_add = (dataSets.data2 >= penBreakBest)'; % 
          end
    else; 
        bool_to_add = bools{2}{j}; % Base level boolean. The dataset corresponding to thisname. 
    end
%     thisname = [thisname '\newline' names{2}{j} ' ' num2str(cut1)]; 
    thisname = sprintf('%s \n %s - %4.0f m \n', thisname, names{2}{j}, cut2)
    thisbool = and(thisbool, bool_to_add ); 
