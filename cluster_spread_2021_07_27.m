...
function dist = cluster_spread(clst, fnew, thistitle, options); 
    arguments
        clst
        fnew
        thistitle
...
if options.showPlot; 
figure(1); clf; set(gcf, 'pos', [2731 1103 496 401]); 
% if size(clst) > 0; 
semilogx(fnew, meanSpec, 'k'); % Actually just plotting thise here to set x log. 
hold on; 
box on; 
grid on ;
title([thistitle sprintf(' - spread of cluster: %5.1f', dist)]); 
